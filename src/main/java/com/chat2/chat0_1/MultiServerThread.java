...
public class MultiServerThread extends Thread {
    ...
    public MultiServerThread(Socket serviceSocket, InetAddress inetAddress, List<Socket> listSockets) {
    ...
    public void run() {
        ...
        try {
            ois = new BufferedInputStream(serviceSocket.getInputStream());

            oos = new BufferedOutputStream(serviceSocket.getOutputStream());
            ...
            while (true) {
                ...
                by[0] = (byte) listSockets.indexOf(serviceSocket);
                //if else 内的数据为服务端接收到的数据,可以
                if (by[1] == 2) {
                    // 因为前两个位置是标志位，所以length的大小为读取的字节数-2,同时offset也从第三个位置(下标是2)开始读
                    String receive = new String(by, 2, res - 2);
                    System.out.println("服务端接收到客户端发来的消息: " + receive);
                    if (receive.equalsIgnoreCase("bye")) {
                        ...
                        oos.flush();
                        System.out.printf("用户%d下线, ", listSockets.indexOf(serviceSocket));
                        listSockets.remove(serviceSocket);
...
//                System.out.println("Socket[]: " + Arrays.toString(listSockets.toArray()));
                ...
                broadCast(serviceSocket, by, res);
            }
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
